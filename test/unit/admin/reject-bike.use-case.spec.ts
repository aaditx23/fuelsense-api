...
import { RejectBikeUseCase } from '../../../src/modules/admin/application/use-cases/reject-bike.use-case';

describe('RejectBikeUseCase', () => {
  let useCase: RejectBikeUseCase;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<RejectBikeUseCase>(RejectBikeUseCase);
  });
  ...
  it('throws when bike is already active', async () => {
    ...
    await expect(useCase.execute(4)).rejects.toThrow(BadRequestException);
  });

  it('rejects and deletes pending bike', async () => {
    repositoryMock.findById.mockResolvedValue({
      id: 4,
      brand: 'Honda',
      model: 'Hornet',
    ...
    expect(result.message).toBe('Bike rejected and deleted successfully');
  });
});
